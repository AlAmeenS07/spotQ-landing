...
// Contact form — validate + submit to Google Sheets
(function initContactForm() {
    ...
    const SHEETS_URL =
        'https://script.google.com/macros/s/AKfycbxgCbKZLFlLU-mH6K-neDMdnAi5Ew-cZx8McqIDe4pjC4ZGEtKlOLXK_TzOUTfWKFGSJQ/exec';

    form.addEventListener('submit', async (e) => {
        e.preventDefault();

        // Collect values from named inputs
        ...
        // Required-field guard (respects novalidate on the form)
        const required = ['restaurant', 'name', 'email', 'message'];
        ...
        try {
            // Use URLSearchParams (simple request) + no-cors to bypass the
            // CORS preflight that Google Apps Script does not handle.
            // In no-cors mode the response is opaque, so we show success
            // optimistically as long as the network call itself succeeds.
            ...
            // If we reach here the request was dispatched without a network error
            success.classList.add('is-visible');
            form.reset();
            success.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        } catch (err) {
            console.error('[SpotQ] Form submission error:', err);
            alert('Unable to send — please check your connection and try again, or email spotqofficial@gmail.com');
        } finally {
